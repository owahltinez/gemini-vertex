...
test('extractModel', async (t) => {
  await t.test('should extract Gemini model names exactly as provided', () => {
  ...
  await t.test('should pass through Claude models untouched', () => {
  ...
  await t.test('should pass through unknown models untouched', () => {
    assert.strictEqual(extractModel('/v1beta1/models/unknown-model-xyz:streamGenerateContent'), 'unknown-model-xyz');
  });
});

test('translateToAnthropic', async (t) => {
  await t.test('should translate basic Gemini message', () => {
    const geminiReq = {
  ...
  await t.test('should translate system instructions', () => {
    const geminiReq = {
      contents: [{ role: 'user', parts: [{ text: 'Hello' }] }],
  ...
  await t.test('should handle JSON response format by appending to system prompt', () => {
    const geminiReq = {
  ...
  await t.test('should filter out empty messages but provide default space if completely empty', () => {
    const geminiReq = {
      contents: [
  ...
  await t.test('should map temperature properly', () => {
    const geminiReq = {
      contents: [{ role: 'user', parts: [{ text: 'Hello' }] }],
