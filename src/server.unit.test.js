...
test('extractModel', (t) => {
  t.test('should extract Gemini model names exactly as provided', () => {
  ...
  t.test('should pass through Claude models untouched', () => {
  ...
  t.test('should pass through unknown models untouched', () => {
    assert.strictEqual(extractModel('/v1beta1/models/unknown-model-xyz:streamGenerateContent'), 'unknown-model-xyz');
  });
});

test('translateToAnthropic', (t) => {
  t.test('should translate basic Gemini message', () => {
    const geminiReq = {
  ...
  t.test('should translate system instructions', () => {
    const geminiReq = {
      contents: [{ role: 'user', parts: [{ text: 'Hello' }] }],
  ...
  t.test('should handle JSON response format by appending to system prompt', () => {
    const geminiReq = {
  ...
  t.test('should filter out empty messages but provide default space if completely empty', () => {
    const geminiReq = {
      contents: [
  ...
  t.test('should map temperature properly', () => {
    const geminiReq = {
      contents: [{ role: 'user', parts: [{ text: 'Hello' }] }],
